...
## To-do list (테스트 케이스 체크리스트 · Coverage 가이드)

### 공통 목표

- 경계값 포함: `0`, `99`, 빈 선호 목록, wrap-around, 세 자리 이상 입력 등
- 라인 커버리지 **80% 이상** 달성
- 모든 예외 케이스에 대한 테스트 작성

### 숫자 버튼 채널 변경

- 한 자리 숫자 입력 + 확인 → 채널 변경
- 두 자리 숫자 입력 → 자동 변경
- 세 자리 이상 입력 → **마지막 두 자리**로 처리
- `0`으로 시작 → 두 번째 자리만 유효 (예: `0`→`7` → 7번)
- 기타 버튼 입력 → 입력 버퍼 무효화
- 유효하지 않은 채널 → 예외 처리
...
- 미등록 채널 → 목록에 추가
- 이미 등록된 채널 → 목록에서 삭제 (토글)
- 여러 채널을 순서대로 추가/삭제하는 시나리오
- 목록 **정렬 순서** 검증
- 최대 저장 개수 경계 (명세에 상한이 있는 경우)
- 목록 조회 시 **불변(immutable) 반환** 여부 확인

### 다음 선호 채널

- 목록 내 현재 채널보다 **큰 값 중 가장 작은** 채널로 이동
- 가장 큰 채널에서 → 가장 작은 채널로 (wrap-around)
- 현재 채널이 목록에 없는 값일 때 동작
- 목록이 비어 있을 때 → 채널 변경 없음

### Tuner Test Double (Fake / Mock)

- 초기 채널값 `0`~`99` 범위 검증
- `setCH()` 유효 값 → 정상 반영
- `setCH()` 유효하지 않은 값 → 예외
- `seekCH()` 10회 연속 호출 → 매번 유효 채널 반환
- `seekCH()` wrap-around 동작 검증
- `getCurrentCH()` 현재 값 반환 검증
